...
**Always build the page using `design-system.md`** (in this skill's directory) — it defines the fixed palette, typography, and component patterns (cards, chips, prompt blocks, quiz questions, timelines) that every artifact from this skill should reuse, so output looks like one consistent product across techniques and across sessions rather than a different improvised style each time. Read that file before writing any HTML. Follow Codex's active frontend and accessibility instructions for fundamentals not already covered there; they override this file where they conflict.

**Always include the "Universal component: inline comments" from `design-system.md`** in every generated HTML, regardless of technique. It lets the user select any fragment and leave a note on it instead of describing the fix in prose, with a "Скопировать для ИИ" button to hand the notes back. Set `data-file` on `<body>` to the path the artifact was actually saved to. If the user later pastes a block starting with "Комментарии из" back into chat, treat each numbered `«quote» — note` line as one localized instruction and address it — same handling as an interview's decisions table, don't ask for clarification unless a note is genuinely ambiguous.

**Copy that component's CSS block and JS block verbatim, both together — do not reimplement, paraphrase, or drop either half.** Paste the whole `<style>` rule set as-is and the whole `<script>` IIFE as-is (adjust nothing but wiring it to the page's own `.page`/`<body data-file>`). A JS-only or markup-only port silently breaks it: without the CSS, `#ai-c-addbtn` has no `position:absolute`/`display:none`, so it renders as a plain unstyled button sitting wherever it falls in normal document flow (typically at the very bottom, after the last section) instead of floating next to the selection — the feature looks like it does nothing, because the button is scrolled far out of view rather than actually hidden. This exact failure has already shipped once from a sub-agent hand-rolling the markup instead of pasting the reference block; treat design-system.md's snippet as copy-paste, not a spec to reimplement from.
